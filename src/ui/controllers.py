...
class AppController:
    ...
    def play_audio(self, path: str):
        ...
        try:
            ...
            threading.Thread(
                target=self._play_thread, args=(data, fs), daemon=True
            ).start()
    ...
    def _train_thread(self):
        ...
        try:
            ...
            X, y, file_paths = load_dataset(
                self.state.target_dir, self.state.others_dir, **mfcc_params
            )
            ...
            if self.state.selected_model_type == "Logistic Regression":
                model.set_params(
                    classifier__C=kwargs.get("C", 1.0),
                    classifier__max_iter=kwargs.get("max_iter", 1000),
                )
            elif self.state.selected_model_type == "SVM":
                model.set_params(
                    classifier__C=kwargs.get("C", 1.0),
                    classifier__kernel=kwargs.get("kernel", "rbf"),
                )
    ...
    def start_recording(self):
        ...
        self.stream = sd.InputStream(
            samplerate=self.fs, channels=1, dtype="int16", callback=callback
        )
        self.stream.start()
    ...
    def stop_recording_and_predict(self, callback_ui=None):
        ...
        threading.Thread(
            target=self._process_audio_thread, args=(callback_ui,), daemon=True
        ).start()
    ...
    def predict_from_file(self, path: str, callback_ui=None):
        ...
        threading.Thread(
            target=self._process_file_thread, args=(path, callback_ui), daemon=True
        ).start()
    ...
    def _process_file_thread(self, path, callback_ui):
        ...
        try:
            _, f_vec = extract_feature_vector_from_file(
                Path(path), **self.state.active_mfcc_params
            )
            prob = get_target_probability(self.state.active_model, f_vec.reshape(1, -1))

            is_target = prob >= 0.5
            self.log(
                f"File {Path(path).name}: {prob * 100:.1f}% -> {
                    'GRANTED' if is_target else 'DENIED'
                }"
            )
    ...
    def _process_audio_thread(self, callback_ui):
        ...
        try:
            _, f_vec = extract_feature_vector_from_file(
                temp_path, **self.state.active_mfcc_params
            )
            prob = get_target_probability(self.state.active_model, f_vec.reshape(1, -1))
