...
class SpeakerVerificationApp:
    ...
    def _create_widgets(self):
        ...
        predict_frame = ttk.LabelFrame(
            main_frame, text=" Prediction & Inference ", padding="10"
        )
    ...
    def _build_training_section(self, parent):
        ...
        ttk.Button(
            data_frame, text="Select Target Folder", command=self.select_target_dir
        ).grid(row=0, column=0, padx=5, pady=2, sticky=tk.W)
        ttk.Label(
            data_frame, textvariable=self.target_dir, width=60, relief="sunken"
        ).grid(row=0, column=1, padx=5, pady=2)

        ttk.Button(
            data_frame, text="Select Others Folder", command=self.select_others_dir
        ).grid(row=1, column=0, padx=5, pady=2, sticky=tk.W)
        ttk.Label(
            data_frame, textvariable=self.others_dir, width=60, relief="sunken"
        ...
        ttk.Label(mfcc_frame, text="Num Coefficients:").grid(
            row=0, column=0, padx=5, pady=2, sticky=tk.W
        )
        ttk.Spinbox(
            mfcc_frame, from_=5, to=40, textvariable=self.num_coefficients, width=8
        ).grid(row=0, column=1, padx=5, pady=2)

        ttk.Label(mfcc_frame, text="Num Filters:").grid(
            row=1, column=0, padx=5, pady=2, sticky=tk.W
        )
        ttk.Spinbox(
            mfcc_frame, from_=10, to=80, textvariable=self.num_filters, width=8
        ).grid(row=1, column=1, padx=5, pady=2)

        ttk.Label(mfcc_frame, text="Frame Size (ms):").grid(
            row=2, column=0, padx=5, pady=2, sticky=tk.W
        )
        ttk.Entry(mfcc_frame, textvariable=self.frame_size_ms, width=10).grid(
            row=2, column=1, padx=5, pady=2
        )

        ttk.Label(mfcc_frame, text="Frame Step (ms):").grid(
            row=3, column=0, padx=5, pady=2, sticky=tk.W
        )
        ttk.Entry(mfcc_frame, textvariable=self.frame_step_ms, width=10).grid(
            row=3, column=1, padx=5, pady=2
        )

        # Model Params
        model_frame = ttk.LabelFrame(
            params_frame, text="Logistic Regression Parameters"
        )
        model_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=(5, 0))

        ttk.Label(model_frame, text="Max Iterations:").grid(
            row=0, column=0, padx=5, pady=2, sticky=tk.W
        )
        ttk.Entry(model_frame, textvariable=self.max_iter, width=10).grid(
            row=0, column=1, padx=5, pady=2
        )

        ttk.Label(model_frame, text="C (Regularization):").grid(
            row=1, column=0, padx=5, pady=2, sticky=tk.W
        )
        ttk.Entry(model_frame, textvariable=self.c_value, width=10).grid(
        ...
        ttk.Entry(exec_frame, textvariable=self.model_name, width=30).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Label(exec_frame, text=".pkl").pack(side=tk.LEFT)

        self.train_btn = ttk.Button(
            exec_frame, text="TRAIN MODEL", command=self.start_training
        )
        self.train_btn.pack(side=tk.RIGHT, padx=5)

        self.train_status = ttk.Label(
            exec_frame, text="Ready", foreground="blue", font=("Helvetica", 10, "bold")
        )
    ...
    def _build_prediction_section(self, parent):
        ...
        ttk.Button(
            top_frame, text="Load Existing Model", command=self.load_model_dialog
        ).pack(side=tk.LEFT, padx=5)
        ttk.Label(top_frame, text="Active Model:").pack(side=tk.LEFT, padx=(20, 5))
        ttk.Label(
            top_frame, textvariable=self.active_model_path, foreground="blue"
        ).pack(side=tk.LEFT)
        ...
        self.record_btn = ttk.Button(
            src_frame, text="🎙️ Record from Microphone (3s)", command=self.record_audio
        )
        self.record_btn.pack(side=tk.LEFT, padx=5, expand=True, fill=tk.X)

        ttk.Button(
            src_frame, text="📁 Load WAV File", command=self.predict_from_file
        ...
        self.prob_label = ttk.Label(
            res_frame, text="Target Probability: --%", font=("Helvetica", 12)
        )
    ...
    def start_training(self):
        ...
        if not target_dir or not others_dir:
            messagebox.showerror(
                "Error", "Please select both Target and Others directories."
            )
            return
        ...
        model_params = {"max_iter": self.max_iter.get(), "C": self.c_value.get()}

        threading.Thread(
            target=self._train_thread,
            args=(target_dir, others_dir, model_name, mfcc_params, model_params),
            daemon=True,
        ).start()

    def _train_thread(
        self, target_dir, others_dir, model_name, mfcc_params, model_params
    ):
        try:
            ...
            save_dict = {"model": model, "mfcc_params": mfcc_params}
    ...
    def _train_success(self, path, save_dict, f1):
        ...
        messagebox.showinfo(
            "Success", f"Model trained and saved successfully.\nF1 Score: {f1:.4f}"
        )
    ...
    def load_model_dialog(self):
        file_path = filedialog.askopenfilename(
            initialdir=MODELS_DIR, filetypes=[("Pickle Files", "*.pkl")]
        )
        if not file_path:
            return
    ...
    def predict_from_file(self):
        file_path = filedialog.askopenfilename(
            initialdir=PROJECT_ROOT, filetypes=[("WAV Files", "*.wav")]
        )
        if not file_path:
            return
    ...
    def _run_prediction(self, wav_path):
        ...
        try:
            ...
            _, feature_vector = extract_feature_vector_from_file(
                wav_path, **self.active_mfcc_params
            )
            ...
            if target_prob >= 0.5:
                self.decision_label.config(
                    text="ACCESS GRANTED (Target)", foreground="green"
                )
                self.log("Prediction: Target Speaker (ACCESS GRANTED)")
            else:
                self.decision_label.config(
                    text="ACCESS DENIED (Other)", foreground="red"
                )
